...
/**
 * Fallback helpers for WP Filebase compatibility when leeno-filebase-compat plugin is inactive.
 */
if ( ! defined('LEENO_FC_ROOT_REL') ) {
...
if ( ! function_exists('leeno_fc_folder_list') ) {
    function leeno_fc_folder_list($cat = 0) {
        ...
        if (!is_dir($path)) {
            // Se la sottocartella specifica non esiste, prova la cartella principale filebase
            $path = leeno_fc_root_dir();
        }
        if (!is_dir($path)) {
            return '';
        }
        ...
        try {
            ...
            foreach ($it as $f) {
                ...
                $fn = $f->getFilename();
                if (strpos($fn, 'thumb_') === 0) {
                    continue;
                }
                $fp = $f->getPathname();
                $relpath = leeno_fc_norm_rel(str_replace(leeno_fc_root_dir(), '', $fp));
                $files[] = [
                    'file_name'         => $fn,
                    'file_display_name' => $fn,
