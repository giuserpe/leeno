...
- **PC `giuserpe`** (nome letterale della macchina): amministratore locale. Qui avvengono commit, push, gestione task Jules, merge PR, review dei diff.
- **PC TEST**: nessun privilegio di amministratore. Usato per test dell'estensione in LibreOffice e, occasionalmente, per editing diretto del codice — MA senza mai fare commit/push da lì.
...
## Ciclo A — Task Jules (gestito interamente da PC `giuserpe`)
