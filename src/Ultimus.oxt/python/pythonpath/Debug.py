...
def aggiorna_configurazione_leeno():
    ...
    if sys.platform == "win32":
        ...
        source_accelerators = r"W:\_dwg\ULTIMUSFREE\_SRC\leeno\src\Ultimus.oxt\Accelerators.xcu"
        sistema = "Windows"
    elif sys.platform == "darwin":  # macOS
        user_config = os.path.expanduser("~/Library/Application Support/LibreOffice/4/user")
        # I path sorgente qui sotto sono placeholder, l'utente dovrà configurarli se usa Mac/Linux per dev
        source_addons = os.path.expanduser("~/path/to/dev/Ultimus.oxt/Addons.xcu")
        source_accelerators = os.path.expanduser("~/path/to/dev/Ultimus.oxt/Accelerators.xcu")
        sistema = "macOS"
    else:  # Linux
    ...
    def process_xcu(source, target, label):
        if not os.path.exists(source):
            return f"✗ {label} sorgente non trovato"
        if not target:
            return f"⚠ {label} target non trovato nella cache"
    ...
    # 5. Elimina registrymodifications.xcu
    ...
    # 6. Messaggio di riepilogo + richiesta riavvio
    msg = (
        f"{risultato_addons}\n"
        f"{risultato_accelerators}\n"
