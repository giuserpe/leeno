...
#     """
#     Analizza il formato numerico di uno stile di cella esistente
#     e restituisce il numero di decimali impostati.
#     """
#     try:
#         doc = LeenoUtils.getDocument()
#         cell_styles = doc.StyleFamilies.getByName("CellStyles")
...
#         format_str = format_entry.FormatString # Es: "#.##0,000"

#         # Se c'è una virgola, contiamo gli zeri dopo di essa
#         if "," in format_str:
#             # Gestisce formati tipo "#.##0,00" -> 2
...
#     """
#     Apre il dialogo IDE per la gestione delle cifre decimali.
#     Legge i dati prioritariamente dal documento aperto.
#     """
#     ctx = LeenoUtils.getComponentContext()
#     smgr = ctx.ServiceManager
...
#     try:
#         # Carica il dialogo disegnato nell'IDE Basic (Libreria UltimusFree2, Nome DlgDecimali)
#         oDlg = dp.createDialog("vnd.sun.star.script:UltimusFree2.DlgDecimali?language=Basic&location=application")
#     except:
#         DLG.chi("Errore: Impossibile caricare il dialogo 'DlgDecimali' dalla libreria 'UltimusFree2'.")
#         return

#     cfg = LeenoConfig.Config()
#     conf_values = cfg.readBlock('DecimaliStili')

#     # Mappatura: Chiave Config -> (Nome Controllo ComboBox nel Dialogo, Nome Stile di Riferimento)
#     mappa_controlli = {
...
#         'quantità': ("cbBLU", "Blu"),
#         'sommano': ("cbSOMMANO", "Comp-Variante num sotto")
#     }
...
#         if ctrl:
#             # 1. Popola la lista della ComboBox
#             ctrl.addItems(lista_decimali, 0)

#             # 2. Determina il valore da mostrare all'avvio
#             # Priorità 1: Documento attivo
#             val_documento = recupera_decimali_da_stile(style_name)

#             if val_documento is not None:
#                 val_iniziale = str(val_documento)
#             else:
#                 # Priorità 2: leeno.conf (fallback su "2")
#                 val_iniziale = str(conf_values.get(key, "2"))

#             ctrl.Text = val_iniziale

#     # Esecuzione del dialogo
#     if oDlg.execute() == 1:
#         nuovi_valori = {}
#         for key, (ctrl_name, _) in mappa_controlli.items():
#             val_scelto = oDlg.getControl(ctrl_name).Text
#             # Validazione: assicuriamoci che sia un numero
#             nuovi_valori[key] = val_scelto if val_scelto.isdigit() else "2"

#         # Salvataggio persistente in leeno.conf
#         cfg.writeBlock('DecimaliStili', nuovi_valori)

#         # Applicazione immediata agli stili del documento
...
#     """
#     Applica i formati numerici agli stili di cella differenziati.
#     Sblocca temporaneamente i fogli protetti.
#     """
#     doc = LeenoUtils.getDocument()
#     num_formats = doc.getNumberFormats()
...
#     # Mappatura categorie -> coppie di stili LeenO
#     config_stili = {
...
#                 # Creazione stringa di formato (es. "#.##0,000")
#                 try:
#                     n_int = int(n)
#                 except:
#                     n_int = 2
...
#                 # Ottieni o crea la chiave del formato
#                 key = num_formats.queryKey(fmt, locale, True)
#                 if key == -1:
#                     key = num_formats.addNew(fmt, locale)

#                 # Applica a tutti gli stili della categoria
