...
#Elenco Prezzi
        ...
        if oDialog1.execute() == 1:
            ...
            if oDialog1.getControl('ComboRAFFRONTO').getText() == 'Variante - Contabilità':
                ...
                for n in range(4, LeenoSheetUtils.cercaUltimaVoce(oSheet) + 1):
                    formule.append([
...
def genera_sommario():
    ...
    with LeenoUtils.DocumentRefreshContext(False):
        ...
        for n in range(4, LeenoSheetUtils.cercaUltimaVoce(oSheet) + 1):
            ...
            stringa = ([
                # Quantità Computo
                f"=LET(s; SUMIF(AA; A{n}; BB); IF(s <> 0; s; \"--\")",
                # Quantità Variante
                f'=LET(s; SUMIF(varAA; A{n}; varBB); IF(s <> 0; s; "--"))' if oDoc.NamedRanges.hasByName('varAA') else '--',
                # Quantità Contabilità
                f'=LET(s; SUMIF(GG; A{n}; G1G1); IF(s <> 0; s; "--"))' if oDoc.NamedRanges.hasByName('GG') else '--',
                '',
                # Scostamento Variante Computo
                f'=LET(s; IF(M{n}="--"; 0; VALUE(M{n})) - IF(L{n}="--"; 0; VALUE(L{n})); IF(s <> 0; s; "--"))',
                # Scostamento Contabilità Computo
                f'=LET(s; IF(N{n}="--"; 0; VALUE(N{n})) - IF(L{n}="--"; 0; VALUE(L{n})); IF(s <> 0; s; "--"))',
                # Scostamento Variante Computo
                f'=LET(s; IF(N{n}="--"; 0; VALUE(N{n})) - IF(M{n}="--"; 0; VALUE(M{n})); IF(s <> 0; s; "--"))',
                '',
                # Importi Computo
                f'=LET(s; IF(L{n}="--"; 0; VALUE(L{n}))*E{n}; IF(s <> 0; s; "--"))',
                # Importi Variante
                f'=LET(s; IF(M{n}="--"; 0; VALUE(M{n}))*E{n}; IF(s <> 0; s; "--"))',
                # Importi Contabilità
                f'=LET(s; IF(N{n}="--"; 0; VALUE(N{n}))*E{n}; IF(s <> 0; s; "--"))',
        ...
        oRange = oSheet.getCellRangeByPosition(11, 3, 21, LeenoSheetUtils.cercaUltimaVoce(oSheet) -1)
        formule = tuple(formule)
        oRange.setFormulaArray(formule)
...
def MENU_analisi_in_ElencoPrezzi():
    ...
    try:
        ...
        oSheet.getCellByPosition(
            1, 4).Formula = "=$'Analisi di Prezzo'.B" + str(riga + 1)
        oSheet.getCellByPosition(
            2, 4).Formula = "=$'Analisi di Prezzo'.C" + str(riga + 1)
        oSheet.getCellByPosition(
            3, 4).Formula = "=$'Analisi di Prezzo'.K" + str(riga + 1)
        oSheet.getCellByPosition(
            4, 4).Formula = "=$'Analisi di Prezzo'.G" + str(riga + 1)
        oSheet.getCellByPosition(
            5, 4).Formula = "=$'Analisi di Prezzo'.I" + str(riga + 1)
        oSheet.getCellByPosition(
            6, 4).Formula = "=$'Analisi di Prezzo'.J" + str(riga + 1)
        oSheet.getCellByPosition(
            7, 4).Formula = "=$'Analisi di Prezzo'.A" + str(riga + 1)
...
def sistema_stili(lrow=None):
    ...
    def _apply_elenco_prezzi_styles(oSheet):
        '''Applica gli stili per il foglio "Elenco Prezzi" in modo ottimizzato.'''
        oRangeAddress = oDoc.NamedRanges.elenco_prezzi.ReferredCells.RangeAddress
        SR = oRangeAddress.StartRow + 1
        ER = oRangeAddress.EndRow
        ...
        # Colonne EP-mezzo
        mezzo_cols = (2, 3, 4, 6, 7, 8, 9)
        for col in mezzo_cols:
            oSheet.getCellRangeByPosition(col, SR, col, ER).CellStyle = 'EP-mezzo'
        # Stili speciali
        oSheet.getCellRangeByPosition(5, SR, 26, ER).CellStyle = 'EP-mezzo %'
        oSheet.getCellRangeByPosition(0, 3, 0, last_row).CellStyle = 'EP-aS'
        oSheet.getCellRangeByPosition(1, 3, 1, last_row).CellStyle = 'EP-a'
        oSheet.getCellRangeByPosition(0, last_row, 9, last_row).CellStyle = 'Riga_rossa_Chiudi'
        # Applicazione stili in batch per migliorare le prestazioni
        oSheet.getCellRangeByPosition(11, SR, 13, ER).CellStyle = 'EP statistiche_q'
        oSheet.getCellRangeByPosition(15, SR, 18, ER).CellStyle = 'EP statistiche_q'
        oSheet.getCellRangeByPosition(19, SR, 21, ER).CellStyle = 'EP statistiche_q'
        return
...
def inizializza_elenco_old():
    ...
    with LeenoUtils.DocumentRefreshContext(False):
        ...
        oSheet.getCellRangeByName('L1').String = ''
        oSheet.getCellRangeByName('P1').String = ''
        oSheet.getCellRangeByName('T1').String = ''
        ...
        oSheet.getCellRangeByName('N3').String = 'Quantità\nContabilità'
        # oSheet.getCellRangeByName('L3:N3').CellBackColor = 16762855
        oSheet.getCellRangeByName('P3').String = 'Scostamento\nVariante\nComputo'
        oSheet.getCellRangeByName('Q3').String = 'Scostamento\nContabilità\nComputo'
        oSheet.getCellRangeByName('R3').String = 'Scostamento\nContabilità\nVariante'
        # oSheet.getCellRangeByName('P3:R3').CellBackColor = 16777062
        oSheet.getCellRangeByName('T3').String = 'Importi\nComputo'
        oSheet.getCellRangeByName('U3').String = 'Importi\nVariante'
        oSheet.getCellRangeByName('V3').String = 'Importi\nContabilità'
        # oSheet.getCellRangeByName('T3:V3').CellBackColor = 16757935
        # oSheet.getCellRangeByName('X3').String = 'Importi\nComputo'
        ...
        for i in (11, 12, 13, 15, 16, 17):
            oSheet.getCellByPosition(i, 1).String = '--'
        oSheet.getCellByPosition(17, 1).String = 'TOTALE'

        oSheet.getCellRangeByName('T2').Formula = '=IF(SUBTOTAL(9;T:T)=0;"--")'
        oSheet.getCellRangeByName('U2').Formula = '=IF(SUBTOTAL(9;U:U)=0;"--")'
        oSheet.getCellRangeByName('V2').Formula = '=IF(SUBTOTAL(9;V:V)=0;"--")'
        oSheet.getCellRangeByName('Y2').Formula = '=IF(SUBTOTAL(9;Y:Y)=0;"--")'
        oSheet.getCellRangeByName('Z2').Formula = '=IF(SUBTOTAL(9;Z:Z)=0;"--")'
        #   riga di totale importo COMPUTO
        y -= 1
        for i in (11, 12, 13, 15, 16):
            oSheet.getCellByPosition(i, y).String = '--'

        oSheet.getCellByPosition(17, y).String = 'TOTALE'
        oSheet.getCellByPosition(19, y).Formula = '=IF(SUBTOTAL(9;T:T)=0;"--")'
        # oSheet.getCellByPosition(12, y).String = 'TOTALE'
        #  riga di totale importo CONTABILITA'
        # oSheet.getCellByPosition(16, y).String = 'TOTALE'
        oSheet.getCellByPosition(20, y).Formula = '=IF(SUBTOTAL(9;U:U)=0;"--")'
        #  rem    riga di totale importo VARIANTE
        # oSheet.getCellByPosition(20, y).String = 'TOTALE'
        oSheet.getCellByPosition(21, y).Formula = '=IF(SUBTOTAL(9;V:V)=0;"--")'
        #  rem    riga di totale importo PARALLELO
        # oSheet.getCellByPosition(23, y).String = 'TOTALE'
        oSheet.getCellByPosition(24, y).Formula = '=IF(SUBTOTAL(9;Y:Y)=0;"--")'
        oSheet.getCellByPosition(25, y).Formula = '=IF(SUBTOTAL(9;Z:Z)=0;"--")'
        oSheet.getCellRangeByPosition(10, y, 26,
                                    y).CellStyle = 'EP statistiche_q'
...
def inizializza_elenco():
    '''
    Riscrive le intestazioni di colonna e le formule dei totali in Elenco Prezzi.
    Ottimizzata per performance e leggibilità.
    '''
    with LeenoUtils.DocumentRefreshContext(False):
        chiudi_dialoghi()
        oDoc = LeenoUtils.getDocument()
        ...
        COLONNE_TOTALI = (11, 12, 13, 15, 16, 17)
        STILI = {
        ...
        # 4. Stili colonne
        oSheet.getCellRangeByName("'Elenco Prezzi'.A2:AA2").CellStyle = STILI['testa']
        oSheet.getCellRangeByName("'Elenco Prezzi'.AA2").CellStyle = STILI['percentuale']
        oSheet.getCellRangeByName("'Elenco Prezzi'.A3:AA3").CellStyle = STILI['intestazione']
        oSheet.getCellRangeByName('I1:J1').Columns.IsVisible = False

        # 5. Configurazione totali
        y = SheetUtils.uFindStringCol('Fine elenco', 0, oSheet) + 1
        
        # Imposta valori iniziali
        for i in COLONNE_TOTALI:
            oSheet.getCellByPosition(i, 1).String = '--'
        oSheet.getCellByPosition(17, 1).String = 'TOTALE'

        # Formule totali
        FORMULE_TOTALI = {
            'T2': '=IF(SUBTOTAL(9;T:T)=0;"--";SUBTOTAL(9;T:T))',
            'U2': '=IF(SUBTOTAL(9;U:U)=0;"--";SUBTOTAL(9;U:U))',
            'V2': '=IF(SUBTOTAL(9;V:V)=0;"--";SUBTOTAL(9;V:V))',
            'Y2': '=IF(SUBTOTAL(9;Y:Y)=0;"--";SUBTOTAL(9;Y:Y))',
            'Z2': '=IF(SUBTOTAL(9;Z:Z)=0;"--";SUBTOTAL(9;Z:Z))'
        }
        ...
        TOTALI_FINALI = {
            17: 'TOTALE',
            19: '=IF(SUBTOTAL(9;T:T)=0;"--";SUBTOTAL(9;T:T))',
            20: '=IF(SUBTOTAL(9;U:U)=0;"--";SUBTOTAL(9;U:U))',
            21: '=IF(SUBTOTAL(9;V:V)=0;"--";SUBTOTAL(9;V:V))',
            24: '=IF(SUBTOTAL(9;Y:Y)=0;"--";SUBTOTAL(9;Y:Y))',
            25: '=IF(SUBTOTAL(9;Z:Z)=0;"--";SUBTOTAL(9;Z:Z))'
        }

        for col, value in TOTALI_FINALI.items():
            oSheet.getCellByPosition(col, y).Formula = value if isinstance(value, str) else value

        oSheet.getCellRangeByPosition(10, y, 26, y).CellStyle = STILI['contab']
...
def vSintetica(flag = True):
    ...
    uscita = False
    # attiva la progressbar
    progress = Dialogs.Progress(Title=f'Elaborazione voci di {oSheet.Name}...', Text=f"Elaborazione di {uRiga} righe ")
    progress.setLimits(0, uRiga)
    progress.show()
...
def trova_np():
    ...
    if Dialogs.YesNoDialog(Title='Nuovi Prezzi',
        Text= 'Vuoi aggiungere il prefisso "NPxx_" ai codici dei nuovi prezzi?') == 1:
        add_prefix = True
    else:
        add_prefix = False

    lrow = SheetUtils.getUsedArea(oSheet).EndRow
    oProgressBar = create_progress_bar(title='Cerco i nuovi prezzi...', steps=lrow)
    ...
    for el in range(3, lrow):
        ...
        val12 = oSheet.getCellByPosition(12, el).Value
        val17 = oSheet.getCellByPosition(17, el).Value
        val21 = oSheet.getCellByPosition(21, el).Value

        if val12 == 0 and (val21 > 0 or val17 > 0):
            current_code = oSheet.getCellByPosition(0, el).String
            
            if add_prefix and not current_code.startswith('NP'):
                # MODIFICA: Gestione del prefisso VDS_
        ...
        if val12 == 0 and val21 > 0:
            cont += val21
            oSheet.getCellRangeByPosition(0, el, 1, el).CellBackColor = 16770000
        elif val12 == 0 and val17 > 0:
            var += val17
            oSheet.getCellRangeByPosition(0, el, 1, el).CellBackColor = 16770002
        else:
            oCellRangeAddr.StartRow = el
            oCellRangeAddr.EndRow = el
            oSheet.group(oCellRangeAddr, 1)
    ...
    if add_prefix and code_mappings:
        oProgressBar.reset()
        oProgressBar = create_progress_bar(title='Propago i nuovi codici...', steps=len(oDoc.Sheets))
        
        for sheet_index, sheet in enumerate(oDoc.Sheets):
            oProgressBar.Value = sheet_index + 1
    ...
    if var > 0:
        Dialogs.Info(
            Title='Informazione',
            Text='''Il totale delle variazioni in VARIANTE è di € {:,.2f}'''
                .format(var).replace(',', 'X').replace('.', ',').replace('X', '.')
        )

    if cont > 0:
        Dialogs.Info(
            Title='Informazione',
            Text='''Il totale delle variazioni in CONTABILITÀ è di € {:,.2f}'''
                .format(cont).replace(',', 'X').replace('.', ',').replace('X', '.')
        )
...
def copia_stili_celle(sheet_src, range_src, sheet_dest, range_dest):
    ...
    progress.setLimits(0, rows)
    progress.setValue(0)
    progress.show()
    # oProgressBar = create_progress_bar(title = 'Importazione stili pagina in corso...', steps = rows)
...
def minuti(arg=None):
    ...
    # Disabilita il rinfresco automatico
    DocumentRefresh(False)
    
    test = getLastUsedCell(oSheet).EndRow+1
    test0 = int(oSheet.getCellRangeByName('AI1').Value)

    def set_common_formulas(y):
        oSheet.getCellByPosition(23, y).Formula = '=O' + str(y + 1)
        oSheet.getCellByPosition(26, y).Formula = '=P' + str(y + 1)

    def process_condition_100(y):
        set_common_formulas(y)
        oSheet.getCellByPosition(20, y).Formula = ''
        oSheet.getCellByPosition(21, y).Formula = ''
        oSheet.getCellByPosition(24, y).Formula = ''
        oSheet.getCellByPosition(25, y).Formula = ''
        oSheet.getCellByPosition(27, y).Formula = '=out1b-in1a+out2b-in2a'

    def process_condition_401(y):
        set_common_formulas(y)
        oSheet.getCellByPosition(22, y).Formula = '=N' + str(y + 1)
        oSheet.getCellByPosition(28, y).Formula = '=in1b-in1a+out1b-out1a+in2b-in2a+out2b-out2a'
        
        testo = oSheet.getCellByPosition(6, y).String
        if len(testo.split()) == 4:
            oSheet.getCellByPosition(27, y).Formula = '=in1b-in1a+out1b-out1a+in2b-in2a+out2b-out2a'
        else:
            oSheet.getCellByPosition(27, y).Formula = '=in1b-in1a+out1b-out1a+in2b-in2a+out2b-out2a'

    def process_condition_402(y):
        set_common_formulas(y)
        oSheet.getCellByPosition(22, y).Formula = '=N' + str(y + 1)
        oSheet.getCellByPosition(23, y).Formula = '=O' + str(y + 1)
        oSheet.getCellByPosition(24, y).Formula = '=IF(X' + str(y + 1) + '<=TIME(15;00;0);TIME(15;00;0);X' + str(y + 1) + ')'
        oSheet.getCellByPosition(25, y).Formula = '=X' + str(y + 1) + '+TIME(3;00;0)'

    conditions = {
        100: process_condition_100,
        401: process_condition_401,
    }
    DocumentRefresh(True)

    formule =[]
    for y in range(test0, test):
        # Imposta formule comuni
        oSheet.getCellByPosition(33, y).Formula = '=IF(AB' + str(y+1) + ';CELL("row");"")'

        if ' ' in oSheet.getCellByPosition(6, y).String:
            try:
                testo = oSheet.getCellByPosition(6, y).String
                for i, orario in enumerate(testo.split()[:6]):
                    oSheet.getCellByPosition(12 + i, y).Formula = '=TIME('+ orario.split(':')[0]+';'+ orario.split(':')[1]+';0)'
            except Exception as e:
                print(f"Errore: {e}")

        if oSheet.getCellByPosition(12, y).Value != 0:
            oSheet.getCellByPosition(18, y).Formula = '=B' + str(y+1)
            oSheet.getCellByPosition(27, y).Formula = '=in1b-in1a+out1b-out1a+in2b-in2a+out2b-out2a'
            oSheet.getCellByPosition(19, y).Formula = '=M' + str(y+1)
            oSheet.getCellByPosition(20, y).Formula = '=IF(in1a>=TIME(7;50;0);in1a;TIME(7;50;0))'
            oSheet.getCellByPosition(22, y).Formula = '=N' + str(y+1)
            oSheet.getCellByPosition(21, y).Formula = '=IF(M' + str(y+1) + '>=TIME(7;50;0);M' + str(y+1) + ';TIME(7;50;0))+TIME(6;0;0)'

        # Applica condizioni
        if oSheet.getCellByPosition(4, y).Value in conditions:
            conditions[oSheet.getCellByPosition(4, y).Value](y)
        elif oSheet.getCellByPosition(0, y).String in ('martedì ', 'giovedì '): 
            process_condition_402(y)

        # Pulisci celle vuote
        for i in range(22, 28):
            if oSheet.getCellByPosition(i, y).Value == 0:
                oSheet.getCellByPosition(i, y).Formula = ''
        formule
        

    # Applica stili e formattazione
    oSheet.getCellRangeByPosition(12, test0, 27, test -1).CellStyle = 'MINUTI'
    oDoc.CurrentController.select(oSheet.getCellRangeByPosition(12, test0, 27, test -1))
    comando('ConvertToValue')

    oSheet.getCellRangeByPosition(18, test0, 26, test -1).CellStyle = 'minuti_bis'
    oSheet.getCellRangeByPosition(18, test0, 18, test -1).CellStyle = 'DATE'

    # Pulisci celle duplicate
    DocumentRefresh(False)

    for y in range(test0, test):
        if 'RPCO' in oSheet.getCellByPosition(6, y).String or 'PDAR' in oSheet.getCellByPosition(6, y).String:
            oSheet.getCellByPosition(27, y).CellBackColor = 16777072
        if oSheet.getCellByPosition(19, y).Value == oSheet.getCellByPosition(20, y).Value:
            oSheet.getCellByPosition(19, y).String = ''
            oSheet.getCellByPosition(20, y).String = ''
        
        if oSheet.getCellByPosition(21, y).Value >= oSheet.getCellByPosition(22, y).Value:
            oSheet.getCellByPosition(21, y).String = ''
            oSheet.getCellByPosition(22, y).String = ''

    # Riabilita il rinfresco automatico
    DocumentRefresh(True)
...
def minuti():
    ...
    num_righe = test - test0
    num_colonne = 23  # Numero di colonne atteso

    dati = []

    for y in range(test0, test):
        riga = [''] * num_colonne  # Assicura che ogni riga abbia il numero corretto di colonne
        ...
        if ' ' in oSheet.getCellByPosition(6, y).String:
            try:
                ...
                for i, orario in enumerate(testo.split()[:6]):
                    ...
                    riga[i] = f'=TIME({ore};{minuti};0)'
            except Exception as e:
                pass
                # DLG.chi(f"Errore elaborazione orario in riga {y}: {e}")

        if oSheet.getCellByPosition(12, y).Value != 0:
            riga[6] = f'=B{y+1}'
            riga[15] = '=in1b-in1a+out1b-out1a+in2b-in2a+out2b-out2a'
            riga[7] = f'=M{y+1}'
            riga[8] = f'=IF(in1a>=TIME(7;50;0);in1a;TIME(7;50;0))'
            riga[10] = f'=N{y+1}'
            riga[9] = f'=IF(M{y+1}>=TIME(7;50;0);M{y+1};TIME(7;50;0))+TIME(6;0;0)'


        riga[21] = f'=IF(AB{y+1};CELL("row");"")'

        cell_value = oSheet.getCellByPosition(4, y).Value
        if cell_value == 100:
            riga[11] = f'=O{y+1}'
            riga[14] = f'=P{y+1}'
            riga[16] = '=out1b-in1a+out2b-in2a'
        elif cell_value == 401:
            riga[10] = f'=N{y+1}'
            riga[11] = f'=O{y+1}'
            riga[14] = f'=P{y+1}'
        ...
        # elif oSheet.getCellByPosition(0, y).String in ('martedì ', 'giovedì '):
            ...
            riga[10] = f'=N{y+1}'
            riga[11] = f'=O{y+1}'
            riga[12] = f'=IF(X{y+1}<=TIME(15;00;0);TIME(15;00;0);X{y+1})'
            riga[13] = f'=Y{y+1}+TIME(3;00;0)'
            riga[14] = f'=P{y+1}'
            riga[15] = '=in1b-in1a+out1b-out1a+in2b-in2a+out2b-out2a'

        dati.append(riga)  # Lista di liste

    DocumentRefresh(True)

    # Range di destinazione
    range_dest = f'M{test0+1}:AI{test}'

    # Controllo che le dimensioni di `dati` siano corrette
    if len(dati) != num_righe or any(len(row) != num_colonne for row in dati):
        DLG.chi(f"Errore dimensioni: {len(dati[1])}x{len(dati[0])} != {num_righe}x{num_colonne}")
        return

    for i, riga in enumerate(dati):
        cell_range = f'M{test0+1+i}:AI{test0+1+i}'
        oSheet.getCellRangeByName(cell_range).FormulaArray = [tuple(riga)]
...
def getColumnWidths(oSheet):
    '''
    Legge tutte le impostazioni delle colonne in modo affidabile
    Restituisce: dict con widths, hidden_ranges, group_levels, visible_cols, freeze
    '''
    def col_index_to_letter(n):
        '''Converte indice colonna (0-based) in lettera (A-Z, AA-AZ, ecc.)'''
        name = ''
        while n >= 0:
    ...
    try:
        ...
        # col_count = cols.getCount()
        col_count = 50
        visible_cols = []
        default_width = cols.getByIndex(0).Width  # Larghezza default
