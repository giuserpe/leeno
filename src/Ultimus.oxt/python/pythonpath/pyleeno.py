...
#Elenco Prezzi
        ...
        comando('Copy')
        oDoc.CurrentController.select(
            oSheet.getCellRangeByPosition(25, 3, 25, ER+2))
        paste_format()
...
def MENU_riordina_ElencoPrezzi():
    ...
    with LeenoUtils.DocumentRefreshContext(False):
        ...
        try:
            ...
            DLG.errore(e)
def MENU_riordina_ElencoPrezzi___():
    """
    Riordina l'Elenco Prezzi secondo l'ordine alfabetico dei codici di prezzo,
    ignorando il prefisso 'VDS_' se presente.
    """
    with LeenoUtils.DocumentRefreshContext(False):
        oDoc = LeenoUtils.getDocument()
        ...
        def get_sort_key(cell):
            """
            Returns the sort key for a cell, ignoring '_VDS' prefix
            """
            value = cell.getString()
            if value.startswith('VDS_'):
                return value[4:]  # Remove the prefix
            return value

        def ordina_intervallo(sheet, start_row, end_row, start_col, end_col):
            """
            Ordina un intervallo di celle per la prima colonna, ignorando '_VDS'
            """
            oRange = sheet.getCellRangeByPosition(start_col, start_row, end_col, end_row)
            
            # Create a list of rows with their sort keys
            rows = []
            for row in range(start_row, end_row + 1):
                cell = sheet.getCellByPosition(start_col, row)
                sort_key = get_sort_key(cell)
            ...
            # Reorder the data according to the sorted rows
            sorted_data = []
            for _, original_row in rows:
                original_index = original_row - start_row
            ...
            for i, row in enumerate(range(start_row, end_row + 1)):
                for j, col in enumerate(range(start_col, end_col + 1)):
                    cell = sheet.getCellByPosition(col, row)
                    cell.setString(sorted_data[i][j])
    ...
    def _apply_elenco_prezzi_styles(oSheet):
        ...
        style_ranges = {
            ...
            'EP statistiche_q': [
                ...
                {'cols': (23, 25), 'rows': (start_row, end_row)}
            ],
            'Riga_rossa_Chiudi': {
...
def struttura_ComputoM():
    ...
    indicator = oDoc.getCurrentController().getStatusIndicator()
    indicator.start('', 4)
    indicator.Text = 'Creazione vista struttura in corso...'
    for n in range(0, 4):
        indicator.Value = n
        struct(n)
    indicator.end()
...
#         val20 = oSheet.getCellByPosition(20, el).Value
#         val21 = oSheet.getCellByPosition(21, el).Value

#         if val19 == 0 and (val21 > 0 or val20 > 0):
...
def trova_np():
    ...
    for el in range(3, lrow):
        current_code = oSheet.getCellByPosition(0, el).String
        if current_code.startswith('NP') and '_' in current_code:
            try:
                np_num = int(current_code[2:current_code.index('_')])
                if np_num > max_np:
                    max_np = np_num
                np_codes_found.append(current_code)
            except ValueError:
                pass
        elif current_code.startswith('VDS_NP') and '_' in current_code[4:]:
            try:
                np_num = int(current_code[6:current_code.index('_', 6)])
                if np_num > max_np:
                    max_np = np_num
                np_codes_found.append(current_code)
            except ValueError:
    ...
    if max_np > 0 and add_prefix:
        ...
        if Dialogs.YesNoDialog(Title='Prefissi NP esistenti trovati',
            ...
            if Dialogs.YesNoDialog(Title='Conferma',
                                  Text='Vuoi resettare la numerazione partendo da NP01_?') == 1:
                np_counter = 1
            else:
                add_prefix = False
                Dialogs.InfoDialog('Operazione annullata', 'Non verranno aggiunti nuovi prefissi NP.')
    ...
    for el in range(3, lrow):
        ...
        if val19 == 0 and (val21 > 0 or val20 > 0):
            ...
            if current_code.startswith('NP') and '_' in current_code:
                try:
                    int(current_code[2:current_code.index('_')])
                    has_np_prefix = True
                except ValueError:
                    pass
            elif current_code.startswith('VDS_NP') and '_' in current_code[4:]:
                try:
                    int(current_code[6:current_code.index('_', 6)])
                    has_np_prefix = True
                except ValueError:
                    pass
        ...
        if val19 == 0 and val21 > 0:
            cont += val21
            # oSheet.getCellRangeByPosition(0, el, 1, el).CellBackColor = 16770000
            oSheet.getCellByPosition(21, el).CellBackColor = 16770000
        elif val19 == 0 and val20 > 0:
            var += val20
            # oSheet.getCellRangeByPosition(0, el, 1, el).CellBackColor = 16770000
            oSheet.getCellByPosition(20, el).CellBackColor = 16770000
        else:
            oCellRangeAddr.StartRow = el
            oCellRangeAddr.EndRow = el
            oSheet.group(oCellRangeAddr, 1)
...
def _propagate_code_changes(oDoc, current_sheet_name, code_mappings):
    '''Propaga i cambiamenti di codice agli altri fogli'''
    oProgressBar = create_progress_bar(title='Propagazione cambiamenti...', steps=len(oDoc.Sheets)-1)
    
    for sheet_index, sheet in enumerate(oDoc.Sheets):
        if sheet.Name != current_sheet_name:
            oProgressBar.Value = sheet_index
...
def _propagate_code_changes(oDoc, current_sheet_name, code_mappings, oProgressBar):
    '''Propaga i cambiamenti di codice a tutti i fogli'''
    oProgressBar.reset()
    oProgressBar = create_progress_bar(
        title='Propago i nuovi codici...', 
        steps=len(oDoc.Sheets)-1  # Esclude il foglio corrente
    )
...
def _show_results(var_total, cont_total):
    ...
    if var_total > 0:
        Dialogs.Info(
            Title='Informazione',
            Text=f"Il totale delle variazioni in VARIANTE è di € {var_total:,.2f}"
    ...
    if cont_total > 0:
        Dialogs.Info(
            Title='Informazione',
            Text=f"Il totale delle variazioni in CONTABILITÀ è di € {cont_total:,.2f}"
                .replace(',', 'X').replace('.', ',').replace('X', '.')
        )
...
def MENU_debug():
    ...
    with LeenoUtils.DocumentRefreshContext(False):
        ...
        oDoc.CurrentController.select(oSheet.getCellRangeByPosition(2, 2, 4, 2))
        # DLG.chi(8)
        # return
        # delete_cells(direction='up')
        # oDoc.CurrentController.select(oDoc.createInstance("com.sun.star.sheet.SheetCellRanges"))  # unselect
        # return
        ...
        lrow = SheetUtils.getLastUsedRow(oSheet)
        indicator.start(f'Cancellazione righe...', lrow)

        for el in reversed(range(0, lrow + 1)):
            indicator.Value = el
            if oSheet.getCellByPosition(0, el).String == '' and oSheet.getCellByPosition(3, el).String == '':
